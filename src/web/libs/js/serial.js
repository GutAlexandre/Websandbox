...
const SerialEvents = Object.freeze({
  ...
  class Serial {
     ...
     */
    async autoConnectAndOpenPreviouslyApprovedPort(serialOptions = { baudRate: 2400 }) {
      if (navigator.serial) {
    ...
    async testwrite() {
      ...
      const bytes = encoder.encode('\x04');
      // console.log(bytes);

      // this.serialWriter.write('\04\r\n');
      console.log(bytes);
     ...
     */
    async connectAndOpen(portFilters = null, serialOptions) {
     ...
     * Defaults to a baud rate of 2400
     * 
